...
if ! timeout -k 2s 10s "$qemu" \
    ...
    show_output
    echo "QEMU failed during VirtIO block test" >&2
    exit 1
fi

if [ "$(grep -cxF 'BoarOS: VirtIO block tests passed' "$output" || true)" -ne 1 ]; then
    show_output
    echo "VirtIO block test kernel did not report success" >&2
    exit 1
fi

if grep -qF 'BoarOS: block test failed' "$output"; then
    show_output
    echo "VirtIO block test kernel reported a failed case" >&2
    exit 1
fi
