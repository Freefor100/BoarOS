...
- Linux 风格系统调用边界显式接收当前调用任务，支持 `exit(93)`、`getpid(172)` 和 `gettid(178)`，未知调用返回 `-ENOSYS`；当前首线程的 TID 与 TGID 相同，取值来自可回收的 1..32768 位图分配器。用户同步故障只终止当前任务并保留 `scause/stval` 完成记录，S-mode 未处理故障仍为内核 fatal。
...
- 自动测试除启动、物理页、分页、Trap、timer 和内核任务状态外，还验证共享 MM 引用、位图 PID 分配，以及记录页访问/释放与页表回收的复合失败，要求状态可重试且页所有权不丢失；两个共享 MM 但使用不同用户栈和身份的任务会真实进入 U-mode，经 timer 抢占到内核 worker 后恢复，核对 `gp/sp/tp/s0..s11`、不同的 `getpid/gettid`、未知 syscall 和 `exit(93)`，另一独立 MM 任务触发 load page fault。另一组测试独立链接完整静态 ELF，验证 `.data`、BSS、参数/环境字符串、auxv、栈对齐、初始提交边界、RX 文本写故障和永久栈 guard 故障。测试最终要求完成记录正确且全部 MM/用户/任务页与 TID 回收，另在用户根下破坏返回凭据验证 fatal 诊断。

当前只支持 RISC-V64 单 hart、QEMU `virt` 平台、Sv39/4 KiB 用户页、S/U 整数 Trap Frame、SBI timer/100 Hz tick、单页内核栈与 FIFO 抢占、每个用户任务为单成员线程组，以及从完整内存缓冲区装载静态 `ET_EXEC` 用户程序。调用者已经能把 acquire 得到的 MM 引用交给多个任务，但尚无从 current task 派生子任务的 `clone` 接口、同组多线程、父子关系、文件表或完整 Linux 进程生命周期；也没有通用用户内存复制、文件来源与 `exec`、动态链接、`fork/wait`、阻塞与唤醒、文件系统、外部中断、SMP 或 LoongArch64。生产启动仍不创建用户任务，完整比赛 Harness 仍会因缺少 `kernel-la` 失败。
