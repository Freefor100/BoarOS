...
- Sv39 建表器按条件组合 2 MiB 与 4 KiB 叶子；最终页表不保留低地址映射，QEMU `virt` UART 的物理 MMIO 通过 `0xffffffe000000000` 的 supervisor-only 高半区别名访问。运行期用户地址空间拥有低半区 4 KiB U 页和页表页、借用包含 UART 在内的最终内核高半区根项，并以 ASID 0 全局刷新方式切换 `satp`；MM 同时维护按地址排序的匿名 VMA，以显式 fault policy 区分必须已驻留的静态 ELF 与 demand-zero 栈/heap。合法匿名缺页分配零页并执行本地单页 `SFENCE.VMA`；heap shrink 先撤销 PTE 并刷新 TLB，再释放页，释放暂时失败的页由 invalid software PTE 保留所有权并在后续增长或 MM 销毁时重试。
...
- Linux 风格系统调用边界显式接收当前调用任务，支持 `openat(56)`、`close(57)`、`read(63)`、`exit(93)`、`uname(160)`、`getpid(172)`、`getppid(173)`、`gettid(178)`、`brk(214)`、普通进程 `clone(220)`、`execve(221)` 和 `wait4(260)`，未知调用返回 `-ENOSYS`。raw `brk` 保留 byte-granular 精确值，增长只登记 demand-zero heap VMA，缩小立即撤销越界页；普通拒绝返回原 break。clone 当前只接受 `SIGCHLD` 进程形态：子 MM eager 深复制，fd 表和 cwd 独立复制，open file description/offset 共享；子进程退出或同步故障产生 Linux 形态 wait status，孙进程在中间父进程退出后 reparent 到 PID 1。每个进程仍是单成员线程组，TID/TGID 来自可回收的 1..32768 位图。
...
当前只支持 RISC-V64 单 hart、QEMU `virt`、Sv39/4 KiB 用户页、S/U 整数 Trap Frame、SBI timer/100 Hz tick、单页内核栈与 FIFO 抢占、单成员线程组、普通 `SIGCHLD` clone，以及 raw whole-disk 只读 ext4 上的静态 `ET_EXEC`。按需分页当前覆盖真实 U-mode 访问触发的匿名栈和 `brk` heap 零页；uaccess 不隐式提交缺页。尚无 COW、`mmap/munmap/mprotect`、`RLIMIT_DATA` 等资源限制、file-backed fault、`CLONE_VM/CLONE_FILES/CLONE_THREAD`、信号投递、futex、vfork、rusage、停止/继续事件或多线程 exec 收拢；ELF 暂不支持 PIE/动态解释器、shebang、TLS 或 `execveat`。文件层没有目录 fd、dup/共享整表、write、页缓存、symlink、分区表或并发访问；也没有并发 unmap 或 SUM 快路径。外部中断、SMP、开发板和 LoongArch64 也尚未实现，完整比赛 Harness 仍会因缺少 `kernel-la` 失败。
...
`make run-riscv` 不附加根盘，启动成功后持续在 timer-idle 中等待，需要由人退出 QEMU。`make test-root-init-riscv` 会建立真实 ext4 镜像，显式启用 modern VirtIO MMIO，把三个测试 ELF 和数据文件写入根盘，并验证连续 exec、参数栈、文件 syscall、退出状态、资源回收和 SBI 关机；`make test-brk-riscv` 汇总 Sv39 unmap、VMA/MM、syscall/ELF 和真实 clone/exec/fault heap 生命周期验证；`make test-demand-page-riscv` 还让 PID 1 在初始栈提交区以下实际触发 load/store 缺页，并用测试内核注入 OOM，验证子进程 wait status 9 和完整回收；`make test-exec-riscv` 运行提交后旧 MM 首次释放失败的故障注入版本；`make test-root-boot-cleanup-riscv` 验证 VMA teardown 在根启动失败中可由持久 owner 重试。`make debug-riscv` 使用 `-S -s` 启动 QEMU：虚拟 CPU 会暂停并在宿主 TCP 端口 1234 等待 GDB，因此命令也不会自行返回。
