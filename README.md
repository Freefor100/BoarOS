...
当前只支持 RISC-V64 单 hart、QEMU `virt`、Sv39/4 KiB 用户页、S/U 整数 Trap Frame、SBI timer/100 Hz tick、单页内核栈与 FIFO 抢占、单成员线程组、普通 `SIGCHLD` clone，以及 raw whole-disk 只读 ext4 上的静态 `ET_EXEC`。首个编译器生成的真实用户程序（静态 musl）已作为 PID 1 在生产内核上运行 stdio、readdir 与文件操作（`make test-userland-riscv`）。按需分页覆盖匿名栈、`brk` heap、private-anonymous mmap 和只读普通文件的 `MAP_PRIVATE`；fork 与文件缓存映射使用 COW。尚无 shared mmap、ASLR、内存承诺/`RLIMIT_DATA`、`CLONE_VM/CLONE_FILES/CLONE_THREAD`、信号投递/handler、futex、vfork、rusage、停止/继续事件或多线程 exec 收拢；ELF 暂不支持 PIE/动态解释器、shebang、TLS 或 `execveat`。文件层没有目录 fd、共享整表、可写文件/writeback、read-ahead、symlink、分区表或并发访问；console read 固定返回空流 EOF。也没有 SMP 并发 unmap/COW、ASID 分配或 SUM 快路径。外部中断、SMP、开发板和 LoongArch64 也尚未实现，完整比赛 Harness 仍会因缺少 `kernel-la` 失败。
...
真实静态 musl 程序已在生产内核上闭环，文件与 stdio ABI 面已达到 musl 静态启动与基础 applet 的下限。近期方向按真实用户程序需求继续：信号投递与默认处置、pipe/poll 等阻塞原语（落地通用等待队列）、终端语义与可写文件；动态 ELF 依赖和多线程共享语义按后续真实用户程序需求排序。RISC-V64 + OpenSBI 主路径稳定后，实现 LoongArch64 16 KiB/三级页表和对应 context/trap；开发板到手后验证固件交接、DTB、设备与真实 TLB/中断性能。
