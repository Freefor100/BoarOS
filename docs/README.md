...
| [RISC-V 启动知识](learning/riscv-boot.md) | ELF、固件交接、地址空间、汇编入口和 QEMU 调试基础 |
| [内存管理知识](learning/memory-management.md) | RAM 发现、保留区、物理页、页表与 Sv39 基础 |
| [第三方代码](third-party.md) | 实际引入的外部源码、版本与许可 |

`modules/` 保存当前实现的稳定事实，接口或不变量变化时同步更新。`learning/` 面向从零开发内核的人，只整理当前主题所需的背景知识、硬件或规范机制、术语和通用调试方法，不记录项目决策、实现状态、计划或历史复盘。根目录 `README.md` 记录当前能力、运行入口和近期方向。每个可独立验证的子系统阶段结束时都要主动检查这三类文档，而不是等到人再次提问，也不是每个提交都追加流水账。
