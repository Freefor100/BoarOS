...
文档按事件产生：模块说明保存当前稳定事实，学习文档整理从零开发当前主题所需的背景知识和通用方法，README 保存当前能力与近期方向。项目决策、实现状态、计划和历史复盘不写入 learning。每个可独立验证的阶段结束时主动检查三者；没有新内容就不制造文档。Harness 或 skill 的临时 plan/spec 默认不进入项目仓库。
