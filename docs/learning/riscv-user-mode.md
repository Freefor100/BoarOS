...
`exit` 只是结束当前可调度任务，不代表 BoarOS 已有完整进程。进程还需要 PID、父子关系、文件表、信号、用户内存复制、ELF 生命周期以及 `fork/exec/wait` 等资源和语义；当前不以空结构伪装这些能力。
...
BoarOS 当前用最小闭环验证架构路径：预映射用户代码/栈、首次 `SRET`、真实 timer 抢占、U-mode syscall、同步页故障、调度恢复和完整资源回收。它没有引入 ELF、进程/PID 或通用 `copy_from_user` 占位；这些能力应在出现真实调用方时形成稳定接口。
