...
本文整理从内核线程走到可执行 U-mode 任务所需的特权级、地址空间、现场切换、系统调用和资源所有权知识，并记录 BoarOS 当前已经验证的选择。稳定接口和限制以 [RISC-V Trap 模块](../modules/riscv-trap.md)、[RISC-V Sv39 分页模块](../modules/riscv-sv39.md)、[RISC-V 用户进程资源模块](../modules/riscv-user-process.md)、[内核线程调度模块](../modules/kernel-scheduler.md)和 [系统调用解码模块](../modules/kernel-syscall.md)为准。
...
线程表示一条可调度执行流，拥有内核栈、Trap Frame 和 switch context；进程表示这条或多条执行流共同使用的资源，至少包含用户地址空间，完整系统还会加入文件表、当前目录、凭据、信号处理方式等。把地址空间直接嵌进线程在“一进程一线程”的早期可以运行，但会把两个生命周期锁死：以后增加共享地址空间的多线程时，要么复制所有权对象，要么大改 scheduler。

BoarOS 当前先建立最小进程资源容器，只拥有一个 Sv39 用户空间，并仍固定一进程一线程。进程记录独占一个 4 KiB 物理页，scheduler 线程只保存可移动 owner 和切换所需的 `satp` 缓存。切换本身直接使用缓存，现有不变量检查会额外解析一次记录来核对它；这不增加页表遍历或 TLB 切换。创建和退出边界由此能独立管理进程资源；以后加入第二个线程或文件表时，扩展的是进程生命周期，而不是 RISC-V switch context 汇编前缀。

所有权转移必须是成功才发生：ELF 装载器先产出 LIVE 地址空间，进程创建成功后把它变成 MOVED，线程创建成功后再把进程句柄变成 MOVED。若记录页已经分配，但访问和立即释放同时失败，CLEANUP 句柄仍精确记住这张页；若地址空间已销毁而记录页释放失败，也用 CLEANUP 只重试最后一页。状态化 owner 比“返回错误码并让调用者猜哪些资源已经释放”更适合内核错误路径。
...
`exit` 只是结束当前可调度任务。BoarOS 已有只拥有地址空间的最小进程资源容器，但这不等于完整 Linux 进程：PID、父子关系、文件表、信号、用户内存复制、可执行文件来源与 `exec` 生命周期以及 `fork/wait` 等资源和语义仍未实现，当前不以空字段伪装这些能力。
...
这一策略要求故障任务拥有独立内核栈和进程地址空间。调度器必须先切到其他可信栈，才能释放故障任务的线程页；用户页表也不能在 `satp` 仍指向它时销毁。BoarOS 的 idle reaper 在内核根页表和 boot stack 上处理完成队列，依次销毁地址空间、进程记录页和线程页。
...
- 创建失败测试既要检查返回错误，也要检查地址空间或进程 owner 仍在调用者手中；记录页访问和释放同时失败时要验证 CLEANUP 可移动并可重试。回收测试比较开始和结束的物理空闲页数，覆盖叶子页、各级页表、进程记录页和线程页。
- “立即释放失败”不能只返回错误码：若新线程页尚未挂入任何队列，scheduler 必须先保存其物理地址，并在清理成功前禁止下一次创建覆盖该 owner。退出回收也要分别在页表、进程记录页和线程页处注入失败，检查完成记录尚未发布且重试从准确阶段继续。
