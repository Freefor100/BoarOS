...
多路复用最初运行于任务控制块与栈共用的 4 KiB 页，控制块与 Trap Frame 会明显挤压调用链预算。当前元数据已独立，执行栈为 8 KiB 连续物理页；容量、测量与静态调用链预算统一见[调度学习记录](kernel-scheduling.md)。

在多路复用实现中，若在栈上静态分配支持 1024 个 fd 的完整位图数组（$6 \times 16 \times 8 = 768$ 字节）以及 16 个描述符节点（约 650 字节），栈帧将迅速超过 1.5 KiB。这会压缩剩余调用链和 Trap 的余量，不能仅因扩大了执行栈就取消堆回退。
...
- **快速路径（Fast-path）微型化**：
  ...
  - 完整余量由编译器报告、调用链分析和真实高水位共同检查，不能由一个函数的帧大小推断。
