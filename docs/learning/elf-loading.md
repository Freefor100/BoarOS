...
因此，先支持“内存缓冲区中的静态 `ET_EXEC`”是一个真实但有限的闭环：它验证 ELF 到地址空间的转换，又不会提前伪造文件系统、动态链接或按需分页接口。以后从文件系统读取完整文件可以直接复用这一路径；当内存占用或启动性能需要优化时，再把数据来源改为逐段读取或文件页映射。
...
最小架构测试只需一张零填充 RW/NX 页和指向页顶的 SP，但 Linux 程序入口通常还期望内核在栈上布置 `argc`、`argv[]`、`envp[]` 和 auxiliary vector。Auxv 会传递页大小、program header 位置与数量、入口、随机字节和平台信息，动态链接器与 libc 启动代码会消费其中一部分。

因此，“能运行不读取启动参数的裸 `_start`”与“能执行普通 Linux 用户程序”之间还有明确一步：定义进程参数所有权、把字符串安全写入用户空间、构造对齐的初始栈，并提供正确 auxv。向下增长的栈应把 guard page 放在栈底以下；若放在栈顶以上，真正的下溢会先写到更低地址的映射，无法形成保护。栈顶 SP 可以等于用户地址上界这个一过尾值，因为实际压栈会先递减再访存。guard 只能让越界尽早 fault；自动增长、多页栈和资源上限属于之后的 VM/进程策略。

## BoarOS 当前选择

通用层只做有界 ELF64 小端字节解析，不引入通用 VM callback 框架。RISC-V 层固定 Sv39/4 KiB，当前只接受静态 `ET_EXEC`，拒绝 `ET_DYN`、`PT_INTERP`、`PT_DYNAMIC` 和 `PT_TLS`；它按页预检 W^X，物化 `PT_LOAD`，在低半区最高一页建立用户栈，并把紧邻下方的一页留作 guard。这个拆分让未来 LoongArch64 能复用 ELF 字节解析，但用 16 KiB/三级页表实现自己的地址布局和权限物化。

当前接口接收完整只读内核缓冲区，装载结束后不持有该缓冲区。成功产出一个独立 LIVE 用户地址空间、入口和 SP，随后 scheduler 才接管地址空间。分配页交给页表之前存在一个需要特别封闭的错误窗口：如果访问该页失败，紧接着释放也失败，那么“调用者持有”与“页表持有”都不成立。BoarOS 把分配、清零、映射和所有权登记收进 Sv39 接口；双重失败时地址空间转为只允许 move/destroy 的 CLEANUP 状态并记录脱离页表树的页。这样所有已分配页始终能从一个可重试对象找到，而不是仅返回状态码。
...
- 最终集成测试应由真实 linker 生成 ELF，并用 `readelf -h/-l/-S` 核对 `ET_EXEC`、机器类型、`PT_LOAD` 权限与 NOBITS BSS。把完整文件送进生产装载器后再实际 `SRET`，才能覆盖解析、物化和硬件执行三层。
