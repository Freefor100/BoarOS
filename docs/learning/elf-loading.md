...
Auxv 的值必须来自真实机制，而不是为了让 libc 继续运行而伪造。`AT_PAGESZ` 描述用户基页大小；`AT_PHDR/AT_PHENT/AT_PHNUM` 描述内存中的 program header table；`AT_ENTRY` 是主程序入口；静态主程序的 `AT_BASE` 为 0；`AT_FLAGS` 当前也为 0。`AT_PHDR` 只有在完整 program header table 随某个 `PT_LOAD` 映入用户空间时才能给出地址，否则只能明确表示不可用。`AT_RANDOM` 需要真正生成并放置随机字节，HWCAP、身份和平台字段也需要各自可靠的数据源；能力尚不存在时省略比填假值更安全。

`argv/envp` 从内核内部创建请求进入装载器时，适合使用“指针 + 显式长度”，因为这能在分配页之前检查总大小、整数溢出和嵌入 NUL。这个结构是内核内部接口，不等于 Linux `execve(const char *path, char *const argv[], char *const envp[])` 用户 ABI；以后实现系统调用时还需要有界地从用户空间逐项复制，再生成同一种内部请求。空参数列表规范化成一个空 `argv[0]`，让入口始终得到可遍历的参数向量。
...
当前接口借用一个精确 read source 和带长度的参数/环境区间，装载结束后不持有文件或内存来源。生产启动把已打开的 ext4 `/init` 转成 source，以 `argv[0]="/init"` 和空环境装载；成功产出独立 LIVE 用户地址空间、入口和 SP，调用者再把地址空间移入 MM，scheduler 最后接管 MM 引用。分配页交给页表之前存在一个需要特别封闭的错误窗口：如果访问该页失败，紧接着释放也失败，那么“调用者持有”与“页表持有”都不成立。BoarOS 把分配、清零、映射和所有权登记收进 Sv39 接口；双重失败时地址空间转为只允许 move/destroy 的 CLEANUP 状态并记录脱离页表树的页。这样所有已分配页始终能从一个可重试对象找到，而不是仅返回状态码。
