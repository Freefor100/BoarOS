...
当前接口接收完整只读 ELF 缓冲区和带长度的参数/环境区间，装载结束后不持有这些缓冲区。成功产出一个独立 LIVE 用户地址空间、入口和 SP；调用者再把地址空间移入进程记录，scheduler 最后接管进程。分配页交给页表之前存在一个需要特别封闭的错误窗口：如果访问该页失败，紧接着释放也失败，那么“调用者持有”与“页表持有”都不成立。BoarOS 把分配、清零、映射和所有权登记收进 Sv39 接口；双重失败时地址空间转为只允许 move/destroy 的 CLEANUP 状态并记录脱离页表树的页。这样所有已分配页始终能从一个可重试对象找到，而不是仅返回状态码。
