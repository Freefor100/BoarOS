...
当前接口借用一个精确 read source 和带长度的文件名/参数/环境区间，装载结束后不持有文件或内存来源。生产启动把已打开的 ext4 `/init` 转成 source；用户 `execve` 则先打开路径、捕获用户字符串，再调用同一装载器。成功产出独立 LIVE 用户地址空间、入口和 SP，调用者把地址空间移入 MM，并由 scheduler 在提交点替换 task 的旧 MM。分配页交给页表之前存在一个需要特别封闭的错误窗口：如果访问该页失败，紧接着释放也失败，那么“调用者持有”与“页表持有”都不成立。BoarOS 把分配、清零、映射和所有权登记收进 Sv39 接口；双重失败时地址空间转为只允许 move/destroy 的 CLEANUP 状态并记录脱离页表树的页。这样所有已分配页始终能从一个可重试对象找到，而不是仅返回状态码。详细装载结果还要与清理结果分开保存，否则一次后续释放失败会掩盖原本应返回给用户的 `ENOEXEC`、`E2BIG` 或 `ENOMEM`。
