...
2. **lwext4 写路径与 POSIX 语义修正**：
   ...
   - 文件大小截断通过 `kernel_vfs_ftruncate` 调用 sparse-capable `ext4_ftruncate`：向下截断释放末尾块；向上截断只清零已分配旧 EOF 块中即将暴露的尾部并更新 inode size，不分配完整中间 gap。越过 EOF 的普通写同样只分配与调用者数据相交的精确逻辑块；部分写入的新块必须先初始化，未映射读取必须由文件层直接合成零，不能让物理块号 0 进入 block I/O。该语义与固定 Linux `references/linux` commit `f4cdf7ca9a1fdcca413157df19753f388a5a224e` 的 `fs/read_write.c::generic_file_llseek_size()`（position 可到 filesystem maxbytes）和 `fs/ext4/inode.c` truncate-up EOF tail zero 路径一致；具体 lwext4 适配基于 `third_party/lwext4/` 固定上游 commit `58bcf89a121b72d4fb66334f1693d3b30e4cb9c5`。
