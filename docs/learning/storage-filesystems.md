...
可写与只读挂载共用同一文件系统路径，差异由块设备写能力和缓存一致性协议决定，涉及块驱动、VFS、文件资源和缓存多个层次：
...
2. **lwext4 写路径与 POSIX 语义修正**：
   ...
   - 文件大小截断通过 `kernel_vfs_ftruncate` 调用 sparse-capable `ext4_ftruncate`：向下截断释放末尾块；向上截断只清零已分配旧 EOF 块中即将暴露的尾部并更新 inode size，不分配完整中间 gap。越过 EOF 的普通写同样只分配与调用者数据相交的精确逻辑块；新块必须在 mapping 可长期保留前整块初始化，初始化失败仅撤销本次 exact block。unwritten extent 的 zero conversion 会留下 dirty cache buffer，因此 caller 部分写不能随后绕过缓存做 direct I/O，否则收尾 flush 会用旧 zero buffer 覆盖 caller bytes；当前数据修改走同一 cache buffer，mapped direct read 前先 flush 对应数据块。未映射读取由文件层直接合成零，不能让物理块号 0 进入 block I/O。maxbytes 必须按 inode mapping 而不是只按 superblock 或 `ext4_lblk_t` 计算：extent inode 的 walker sentinel 给出 `EXT_MAX_BLOCKS * block_size`，legacy inode 只能覆盖 `inode_block_limits[3] * block_size`；该上限在尾部清零和 offset 缩窄前检查。该语义与固定 Linux `references/linux` commit `f4cdf7ca9a1fdcca413157df19753f388a5a224e` 的 `fs/attr.c::inode_newsize_ok()`、`fs/read_write.c::generic_file_llseek_size()` 和 `fs/ext4/inode.c` truncate-up EOF tail zero 路径一致；具体 lwext4 适配基于 `third_party/lwext4/` 固定上游 commit `58bcf89a121b72d4fb66334f1693d3b30e4cb9c5`。
   ...
   - 任务内核栈深约束：BoarOS 的 4 KiB 任务页被 `struct kernel_task`（约 1.5 KiB）和内核执行栈共用，留给系统调用与 trap 处理的栈空间仅约 2.5 KiB。用户态系统调用进入内核时，288 字节的 RISC-V trap frame 已经占用任务内核栈；在该 trap frame 存续期间，VFS/lwext4 的深层调用继续消耗同一任务栈预算，而 S-mode trap 入口会清除 `sstatus.SIE`，因此这里不依赖“定时器在深层 syscall 内再次嵌套”的模型。应避免在栈上直接实例化 304 字节的 `ext4_dir` 或额外的 264 字节 `ext4_direntry`；目录文件句柄打开采用专有的 `ext4_dir_open_file` 直连 `node->file`，目录项遍历直接复用 `directory.de`，并在 `unlink` 前消除重复的路径属性查找（`ext4_mode_get`）。若深层文件系统路径耗尽剩余栈空间并破坏 canary，该损坏可能在系统调用返回后、后续正常定时器中断触发 scheduler validation 时才被检测到。
