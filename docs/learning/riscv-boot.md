...
分页关闭时 CPU 不能直接取高虚拟地址。BoarOS 使用 `-mcmodel=medany` 生成以 PC 相对寻址为主的早期代码，使同一内核镜像在低物理装载地址建立栈、清零 BSS、读取 DTB 并创建页表。页表同时映射低地址执行路径和高半区内核；`satp` 生效后，再把返回地址、栈、`gp`、`stvec` 与控制流切换到高地址。
