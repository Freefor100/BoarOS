...
- 分页开启后，分配器托管的 RAM 必须存在可访问的内核映射，否则释放链表会在读写节点时出错；BoarOS 已由 Direct Map 承担分页后的页内容访问，RAM 恒等映射只保留为首次 `satp` 切换的临时执行与栈桥梁，高半区内核映射、Direct Map 和恒等映射的职责不能混为一谈。
...
Sv39 建表测试覆盖精确 PTE、2 MiB/4 KiB 选择、16 GiB 规模、边界拒绝和部分提交；权限故障测试覆盖 MMU 生效后的只读保护；完整启动测试再验证 512 MiB/1 GiB RAM 下的 `satp`、页表页计数和高半区执行上下文；高半区 trap 测试验证硬件实际使用迁移后的 `stvec`。开发板到手后还必须补充同类硬件验证，不能把 QEMU 结果直接等同于板级兼容。
