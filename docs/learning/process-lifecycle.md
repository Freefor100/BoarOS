...
- 已由 wait 摘除的对象不再有父关系或 PID，只能在任务栈仍使用任务页时短暂位于静默 EXITED 队列；
- boot completion 只属于启动路径直接创建的顶层任务，普通子进程由 wait 消费。

这种状态树比按 `if/else` 数量判断覆盖更可靠；测试应核对最终 PID、heap 和物理页基线，并把 allocator 非法释放与真实文件系统 I/O 清理错误分别验证，避免为生产路径制造伪造的释放重试。
