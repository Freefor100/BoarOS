...
- 固定真实程序清单已落地（`make inventory-userland-riscv`）：固定 `pre-2025` commit 的 libc-test 与完整 398 applet BusyBox 已按原配置构建。路径链接、同步信号等待及 `prlimit64` 后，原 226 项加两个独立 BusyBox `od/hexdump` 共 228 项双侧运行；Linux 228/228 满足契约，BoarOS 211 项双侧一致、14 项直接 entry 退出不符、3 项原脚本断言失败。原 BusyBox 脚本最终复跑 45/55 success，后台 sleep+kill 在两次运行间一过一败、尚需定位；静态/动态 libc 包装脚本分别完整运行 107/110 项，其中 100/103 项通过断言，余下七个失败名称相同。`rlimit_open_files`、`sscanf_long` 已随资源限制查询/设置恢复；`pthread_cancel_points` 直接 entry 本次重复通过，但先前失败没有独立根因。余下依赖 socket、robust futex、设备节点/身份、显式设置时间、`chdir` 和 statvfs，逐例证据见[程序清单](learning/user-program-inventory.md)。不把有限命令或包装器通过等同于完整程序兼容。后续 pthread stress、SQLite、Git 须各自建立最小复现与 Linux 差分。
