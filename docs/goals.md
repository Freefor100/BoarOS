...
- 固定真实程序清单已落地（`make inventory-userland-riscv`）：已从固定 `pre-2025` commit 补齐 libc-test，完整 398 applet BusyBox 以固定 UAPI 原配置构建；逐例运行及原脚本的证据与边界见[程序清单](learning/user-program-inventory.md)。补齐环境后的 226 项 Linux 均满足契约，BoarOS 104 项双侧一致、119 项退出不符、3 项脚本断言失败（BusyBox 原脚本 42/55，libc 静态 96/107、动态 2/110）。环境修复不能计为内核能力完成；先定位实际失败簇，再按 libc-test、BusyBox、pthread stress、SQLite、Git 推进。ELF 文件尾页/BSS 边界已由真实双侧复现修复：动态 libc 直接用例现为 99/110 双侧一致，剩余 11 项与静态失败集合相同；下一步补齐 readv 并重跑完整清单，历史 2/110 是修复前基线；此外保留 proc/sysfs、设备/链接、目录操作、身份/rlimit、时间设置与官方 runtest 的依赖清单。每轮按 failure cluster、最小复现、Linux differential、kernel primitive regression、重跑 workload 闭环；SQLite 的验收包含事务、journal、并发 reader/writer、重开和 integrity_check。
