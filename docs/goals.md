...
| 后续 Linux 能力组 | 动态链接、更多文件 syscall、实时信号排队与 `sigaltstack`/signalfd、线程/共享资源（进一步扩展）、外部中断、SMP、可写文件系统及更多设备 transport | 属于长期 ABI 路线，按真实用户程序和依赖推进，不能伪装成当前已支持 |
