...
- 固定真实程序清单已落地（`make inventory-userland-riscv`）：固定 `pre-2025` commit 的 libc-test 与完整 398 applet BusyBox 已按原配置构建。ELF 文件尾页/BSS 与 `readv` 修复后，原 226 项加独立 BusyBox `od/hexdump` 两项完成 Linux/BoarOS 逐例运行；Linux 228/228 满足契约，BoarOS 205 项双侧一致、20 项直接 entry 退出不符、3 项原脚本断言失败。静态 libc 为 97/107、动态为 100/110，剩余十个失败名称相同；原 BusyBox 脚本由 42/55 增至 43/55 success，两个新增命令完整输出双侧一致。证据与失败名称见[程序清单](learning/user-program-inventory.md)。原 libc 包装器仍缺 `sigtimedwait`，直接 entry 的失败还涉及 rlimit、robust futex、socket、statvfs、设备节点、身份及设置时间；`pthread_cancel_points` 需另做聚焦复现。先按真实失败簇选择下一能力，不把环境或包装器失败视为所有直接案例失败。后续 pthread stress、SQLite、Git 须各自建立最小复现与 Linux 差分。
