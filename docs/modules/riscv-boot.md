...
当前代码只处理单 hart、固定 QEMU `virt` UART、固定高半区内核 VMA、最终 high/direct RAM 映射、不可恢复的致命 trap、第一段 DTB 物理内存和静态保留区，以及最小单页分配。QEMU ELF 的物理装载地址仍固定为 `0x80200000`；VisionFive 2 的装载地址和固件入口必须在板级适配时单独提供，不能直接沿用该平台常量。运行期映射修改、中断、用户态 trap 或 LoongArch64 尚未实现；额外挂载的 VirtIO 块设备和网卡尚未访问。
