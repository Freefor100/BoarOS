...
当前代码只处理单 hart、固定 QEMU `virt` UART、直接物理地址、不可恢复的致命 trap、第一段 DTB 物理内存和静态保留区。它尚未处理多段 RAM、页分配、中断、分页、用户态 trap 或 LoongArch64；额外挂载的 VirtIO 块设备和网卡尚未访问。
