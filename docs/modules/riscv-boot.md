...
- 链接脚本把 `_start` 和第一个 `PT_LOAD` 放在 guest 物理地址 `0x80200000`。该地址位于 QEMU `virt` 从 `0x80000000` 开始的 RAM 中；首次启用分页前使用物理地址，启用后由 Sv39 恒等映射保持同一数值地址可用。
...
- `kernel_main` 在分页关闭时从分配器取得页表页，将 RAM 恒等映射为 text RX、rodata R 和其余 RAM RW，并将 QEMU UART 恒等映射为 RW。随后写入 `satp.MODE=8`；正常启动日志中的页表数必须等于物理页总数与可用页数之差。
...
当前代码只处理单 hart、固定 QEMU `virt` UART、启动期恒等映射、不可恢复的致命 trap、第一段 DTB 物理内存和静态保留区，以及最小单页分配。它尚未处理高半区、运行期映射修改、中断、用户态 trap 或 LoongArch64；额外挂载的 VirtIO 块设备和网卡尚未访问。
...
`make test-riscv` 依次运行 DTB/布局、物理页分配、Sv39 建表和只读权限故障聚焦测试，再在 512 MiB 与 1 GiB 下各启动一次正常内核。完整启动要求 hart ID 为 0、DTB 地址随 RAM 大小变化、解析出的基址恒为 `0x80000000`、大小分别为 `0x20000000` 与 `0x40000000`、`satp.MODE=8`、页表计数一致，且 QEMU 经 SBI 正常退出。`make debug-riscv` 会在第一条 guest 指令前暂停，并在宿主 TCP 端口 1234 等待 GDB。
