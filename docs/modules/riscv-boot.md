...
本文描述当前代码的稳定事实。构建、地址与固件交接的个人理解见 [RISC-V 启动学习记录](../learning/riscv-boot.md)，异常诊断契约见 [RISC-V 致命 Trap 模块](riscv-trap.md)。
...
当前代码只处理单 hart、固定 QEMU `virt` UART、直接物理地址和不可恢复的致命 trap。它没有中断、分页、内存分配、设备树解析、用户态 trap 或 LoongArch64 支持；额外挂载的 VirtIO 块设备和网卡尚未访问。
