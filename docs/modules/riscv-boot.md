...
本文描述当前代码的稳定事实。构建、地址空间、固件交接和项目选择的解释见 [RISC-V 启动学习总结](../learning/riscv-boot.md)，异常入口与返回契约见 [RISC-V Trap 模块](riscv-trap.md)，启动内存边界见 [DTB 与启动内存布局模块](dtb-memory.md)，分页切换见 [RISC-V Sv39 分页模块](riscv-sv39.md)。
...
| `tests/boot-riscv.sh`、`tests/high-half-trap-riscv.sh`、`tests/no-identity-riscv.sh` | 验证 ELF/运行时地址契约、两种内存配置下的启动链、高半区 trap 和最终低 RAM 失效 |
...
- 启动栈可用后，`_start` 将 `sscratch` 清零并把 `riscv_trap_entry` 写入 Direct-mode `stvec`。入口可以在当前内核栈保存完整整数 Frame 并执行 `sret`；当前生产 dispatcher 尚无正式 handler，所以未处理 trap 仍进入致命诊断。
...
当前代码只处理单 hart、固定 QEMU `virt` UART、固定高半区内核 VMA、最终 high/direct RAM 映射、S-mode 整数 Trap Frame/返回、第一段 DTB 物理内存和静态保留区，以及最小单页分配。QEMU ELF 的物理装载地址仍固定为 `0x80200000`；VisionFive 2 的装载地址和固件入口必须在板级适配时单独提供，不能直接沿用该平台常量。生产中断 handler、用户态 trap、运行期映射修改或 LoongArch64 尚未实现；额外挂载的 VirtIO 块设备和网卡尚未访问。
