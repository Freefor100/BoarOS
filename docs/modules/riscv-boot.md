...
本文描述当前代码的稳定事实。构建、地址与固件交接的个人理解见 [RISC-V 启动学习记录](../learning/riscv-boot.md)，异常诊断契约见 [RISC-V 致命 Trap 模块](riscv-trap.md)，DTB 扫描边界见 [DTB 物理内存发现模块](dtb-memory.md)。
...
| `kernel/dtb.c` | 校验 DTB 并读取第一段物理内存 |
| `kernel/main.c` | 读取并输出 DTB 物理内存范围后关机 |
...
- DTB 地址不是常量。`kernel_main` 通过 `dtb_read_first_memory_range` 扫描根节点的 `memory` 子节点，并拒绝畸形、缺失或当前不支持的内存描述。
...
当前代码只处理单 hart、固定 QEMU `virt` UART、直接物理地址、不可恢复的致命 trap 和第一段 DTB 物理内存。它尚未处理 DTB 保留区、多段 RAM、中断、分页、内存分配、用户态 trap 或 LoongArch64；额外挂载的 VirtIO 块设备和网卡尚未访问。
...
`make test-riscv` 先运行 DTB 聚焦解析测试，再在 512 MiB 与 1 GiB 下各启动一次正常内核，要求启动行仅出现一次、hart ID 为 0、DTB 地址随 RAM 大小变化、解析出的基址恒为 `0x80000000`、大小分别为 `0x20000000` 与 `0x40000000`，且 QEMU 经 SBI 正常退出。`make debug-riscv` 会在第一条 guest 指令前暂停，并在宿主 TCP 端口 1234 等待 GDB。
