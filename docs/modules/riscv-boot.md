...
- `kernel_main` 在分页关闭时从分配器取得页表页，将 RAM 恒等映射为 text RX、rodata R 和其余 RAM RW，同时把内核镜像按相同物理页与权限映射到 `0xffffffff80000000`，并将 QEMU UART 恒等映射为 RW。
- 写入 `satp.MODE=8` 后，低地址恒等映射先保证返回路径仍可执行；`riscv_relocate_to_high` 再把 `ra`、`sp`、`stvec` 加上固定偏移，跳到高地址代码并重新建立高地址 `gp`。此后正常内核路径在高半区执行，DTB、页表页和 UART 暂时仍通过低地址恒等映射访问。
...
当前代码只处理单 hart、固定 QEMU `virt` UART、固定高半区内核 VMA、启动期高低双映射、不可恢复的致命 trap、第一段 DTB 物理内存和静态保留区，以及最小单页分配。QEMU ELF 的物理装载地址仍固定为 `0x80200000`；VisionFive 2 的装载地址和固件入口必须在板级适配时单独提供，不能直接沿用该平台常量。物理内存 direct map、移除恒等映射、运行期映射修改、中断、用户态 trap 或 LoongArch64 尚未实现；额外挂载的 VirtIO 块设备和网卡尚未访问。
