...
当前同时接受 VirtIO MMIO version 1 legacy 和 version 2 modern。modern 路径要求协商 `VIRTIO_F_VERSION_1` 并检查设备回显 `FEATURES_OK`；legacy 路径使用传统 feature 寄存器和状态序列，不伪造 modern feature。设备状态按各自规范推进，失败和销毁会复位设备后再回收队列页。QEMU 默认 legacy 和显式 `-global virtio-mmio.force-legacy=false` 的 modern 配置都由同一驱动验证。

## 请求和 DMA

通用 `kernel_block_device` 暴露容量、逻辑块大小和精确字节范围的同步只读接口，统一检查空参数、越界和整数溢出。两种 transport 共用 size 8 split queue、三描述符请求链、状态字和 512 字节 bounce buffer；modern 队列占用一张 4 KiB 页，legacy 采用 4 KiB `GuestPageSize`/`QueueAlign`、连续对齐的 16 KiB 分配，并把 used ring 放在下一对齐边界。当前同一时刻只有一个请求。
