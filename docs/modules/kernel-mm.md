...
`kernel_mm_fork()` 先克隆所有会分配的 VMA 和文件来源 metadata，最后才调用 Sv39 两阶段 fork。子页表先取得每个 active/`PROT_NONE` 4 KiB 页的物理引用并以 COW 形态建立；全部构造成功后再把父进程原可写叶子去掉 W 并标记 COW，随后刷新父地址空间 TLB。metadata 或子页表构造失败时父 PTE 不变；父提交阶段不再分配。retired 页不进入子地址空间，内核高半区仍只借用稳定映射。
...
创建失败时，目标句柄要么仍为 EMPTY，要么成为持有精确剩余 owner 的 CLEANUP；调用者必须按状态继续释放，不能只按错误码假定没有分配。Fork 仍需遍历已提交页和构造子页表，时间与驻留页数线性，但不再在 fork 时复制每页内容或预留同量数据内存；额外数据页成本只落在之后真正写入的页。当前 clone 仍在单 hart 关中断临界区内完成 walker 和 metadata 分配，大地址空间的最坏中断延迟尚需开发板测量。SMP 时还必须用 MM 锁、原子页引用和远端 TLB shootdown 保护父提交与并发 fault/unmap。
...
VMA metadata 释放失败时转入 `CLEANUP_VMAS`；成功后 `CLEANUP_FILE_SOURCES` 逐个释放 MM 持有的 OFD 和注册节点，再进入 Sv39 回收。Sv39 销毁一旦发生部分提交，MM 转入 `CLEANUP_SPACE`；地址空间已经销毁而记录页释放失败时转入 `CLEANUP_RECORD`。后续调用只重试当前阶段。所有 CLEANUP 都可 move，不能 acquire、lookup 或生成 `satp`。没有 VMA 的既有 MM 若 Sv39 销毁未发生提交即失败，则仍保持 LIVE；非末引用释放不触碰 VMA、文件来源或页表树。
