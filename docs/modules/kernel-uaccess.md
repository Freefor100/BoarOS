...
合法范围按 4 KiB 基页边界切分。每个片段先经 `kernel_mm_lookup()` 查询物理地址和通用权限：写入用户空间要求 `USER|WRITE`，读取用户空间要求 `USER|READ`，随后通过物理页分配器已绑定的 direct-map 访问函数复制。未映射或权限不足返回 `FAULT`；MM 状态、页表结构或已映射物理页无法解析返回 `STATE`，不能伪装成用户 `EFAULT`。
