...
| 文件 | 当前职责 |
|---|---|
| `include/kernel/vma.h`、`mm/vma.c` | 定义 VMA 描述符、按地址排序的集合及其创建、复制、末端裁剪、查询和销毁 |
...
一个 VMA 用半开区间 `[start, end)`、R/W/X 权限、kind、role、fault policy 和可选 backing 表示一段**逻辑有效**的用户虚拟地址。PTE 只表示其中当前已经驻留的 4 KiB 页；没有 PTE 不等于没有 VMA。`RESIDENT_REQUIRED` 要求合法页已经由装载路径物化，`DEMAND_ZERO` 允许硬件用户缺页按 VMA 权限建立匿名零页。栈 VMA 覆盖完整 8 MiB reserve，但初期只有靠近栈顶的一小段有 PTE；`brk` heap VMA 则只覆盖当前 break 向上按页对齐后的范围。两者未触碰的页都在用户 load/store 首次访问时分配。guard 页不属于任何 VMA，访问它继续按未映射处理。
...
MM record 拥有 VMA 集合，而不是 task 或 Sv39 page-table object。集合借用启用时传入的 kernel heap；该 heap 必须在 MM 及其全部 cleanup retry 之后才结束生命周期。`kernel_mm_fork()` 在成功深复制用户页之后复制 VMA 描述符，因此子进程的逻辑地址布局与父进程独立但初始相同；当前匿名描述符不持有外部 backing。`kernel_vma_set_trim_end()` 只接受完整匹配的现有 VMA，并把它的高端收缩到明确的新边界；边界等于起点时删除描述符。这是当前 `brk` shrink 的受限内部原语，不提供任意 split 或用户 `munmap` 语义。若 fork 复制失败且内部回收成功，输出目标恢复为可复用的 `EMPTY`；若仍有资源无法回收，输出才成为精确的 `CLEANUP` owner。最后一个 MM owner 的清理顺序固定为：
...
新 ELF 登记静态段和栈后初始化 page-aligned heap 起点；空 heap 没有 VMA。raw `brk` 跨页增长插入相邻、同属性的 RW anonymous `HEAP/DEMAND_ZERO` 区间，集合会把它与既有 heap 合并，因此 VMA 数量不会随每次小幅增长线性增加。普通 byte-granular 调整若仍落在同一页，只更新 MM 中的精确 break，不改 VMA。

缩小先由 Sv39 后端让被撤销 PTE 对硬件失效并执行 TLB fence，随后用末端裁剪把 VMA 收缩到 `page_end(new_brk)`。这条顺序保证 VMA 删除后不会残留硬件仍可访问的 active PTE；页释放失败只会留下 invalid retired PTE，不重新扩大 VMA。当前单 hart、关中断执行使 PTE 与 VMA 提交不可被另一个 hart/线程观察；SMP 必须用 MM 锁与远端 shootdown 扩展该不变量。
...
当前匿名栈和 `brk` heap 使用 demand-zero；没有 `mmap`、用户 `munmap`、`mprotect`、COW、file-backed 缺页/VMA 引用或 SMP 并发修改。VMA 操作和缺页解析由当前单 hart、关中断的任务生命周期串行化；接入 SMP 前必须为 VMA 查改、fork、最终清理和远端 TLB shootdown 定义锁与引用协议。
