...
RISC-V 在首次启用分页前以物理地址访问分配器和页表页，启用后由 Sv39 恒等映射
覆盖全部托管 RAM，因此回收节点仍可按同一数值地址访问。页表页来自该分配器，
每取得一页都会相应减少可用页计数。LoongArch 接入时需按其直接映射窗口增加
真实的架构地址转换。模块尚不支持连续多页、清零分配、伙伴系统、并发、NUMA
或热插拔。
...
配置下要求初始化后的总页数非零；启用 Sv39 后，总页数与空闲页数的差必须等于
建表器报告的页表页数。
