...
| 文件 | 职责 |
|---|---|
| `kernel/exec.c`、`include/kernel/exec.h` | 捕获用户路径/argv/envp，打开可执行文件，解析并持有主程序和解释器 source，管理可重试事务 |
...
事务中的 `executable_file`/`interpreter_file` 是 source 创建前的 OFD owners；source 创建成功后由 source 持有 OFD，事务改为持有 source owner。映像构造给 MM 增加各 source 的引用，事务在提交后释放自己的引用。任意 OFD、source、heap、VMA 或新 MM 清理失败都会保留准确 owner，事务挂在 task 上供下次 exec 或退出 reaper 重试。
...
提交前 scheduler 验证当前 task、单成员线程组、新 MM 的 `satp`、入口可执行 VMA/PTE、栈 RW PTE 和 SP 对齐。随后：

```text
切换 new satp
-> 将 old MM 移入 retired owner
-> 安装新 MM 和缓存 satp
-> 清零并重建 Trap Frame
-> 摘除 FD_CLOEXEC
-> 清理 retired MM、source 和事务 owner
-> sret 到新入口
```

提交后不撤销新映像。PID/TID、cwd、files、fs context 和非 `FD_CLOEXEC` fd/OFD offset 保留；信号 disposition 按 exec 规则重置，F/D 状态清零。清理失败不伪造旧程序的 errno，而由持久 owner 继续重试。
...
静态 fixture 仍用于基础回归，不能代表动态能力上限。动态 ET_DYN/解释器的生产构造已经接入；musl/glibc 重定位、额外 DSO、TLS、多线程 exec、shebang、`execveat`、凭据变化、写入文件的一致性和 LoongArch 后端仍未完成。128 KiB 是当前序列化初始栈镜像限制，不是完整 Linux `ARG_MAX` 策略。
